  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{display:"flex",background:"#fff",borderBottom:"2px solid #f0f0f0",overflowX:"auto"}}>
        {TABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"12px 4px",border:"none",background:"none",color:tab===t.id?R:"#444",fontWeight:tab===t.id?800:600,fontSize:12,cursor:"pointer",borderBottom:tab===t.id?`3px solid ${R}`:"3px solid transparent",whiteSpace:"nowrap",minWidth:72}}>{t.label}</button>)}
      </div>
      <div style={{padding:"16px 20px 100px"}}>
        {tab==="companies"&&<>
          <p style={{fontSize:13,color:"#444",margin:"0 0 12px",fontWeight:500}}>Tap <strong>Edit</strong> to add address, phone, email, text, primary contact, and billing/AP details.</p>
        ...
        {tab==="drivers"&&<>
          <p style={{fontSize:13,color:"#444",margin:"0 0 12px",fontWeight:500}}>Tap <strong>Edit</strong> to add address, phone, email, text, CDL #, and emergency contact.</p>
        ...
        {tab==="quarries"&&<>
          <p style={{fontSize:13,color:"#444",margin:"0 0 12px",fontWeight:500}}>Tap <strong>Edit</strong> to add main office, accounts payable, and scale/dispatch contacts.</p>
        ...
        {tab==="reports"&&<>
          <div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}>
            <p style={{margin:"0 0 10px",fontWeight:800,fontSize:12,color:"#444",letterSpacing:1}}>TIME PERIOD</p>
          ...
          {Object.keys(byDriver).length>0&&<div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 2px 10px rgba(0,0,0,0.08)"}}>
            <h3 style={{margin:"0 0 12px",fontSize:12,color:"#444",fontWeight:800,letterSpacing:1}}>DRIVER BREAKDOWN</h3>
          ...
          <div style={{background:"#fff",borderRadius:14,padding:16,marginTop:16,border:"2px dashed #d1d5db"}}>
            <p style={{margin:"0 0 6px",fontWeight:800,fontSize:12,color:"#444",letterSpacing:1}}>🧪 DEMO / SAMPLE DATA</p>
            ...
            {workOrders.filter(w=>w.isSampleData).length>0&&<p style={{margin:"10px 0 0",fontSize:12,color:"#059669",textAlign:"center"}}>✓ {workOrders.filter(w=>w.isSampleData).length} sample tickets loaded</p>}
          </div>
        </>}
      </div>
      {editModal&&(
      ...
      <div style={{padding:"16px 20px 100px"}}>
        ...
        <Section title="MATERIAL HAULED" icon="⛏️">
          <Field label="Material Category" err={errors.topMaterial}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:12}}>
              {TOP_MATERIALS.map(m=><button key={m} onClick={()=>setMany({topMaterial:m,materialHauled:"",quarry:"",quarryKnown:null,quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:""})} style={{padding:"10px 8px",border:`2px solid ${form.topMaterial===m?R:"#bbb"}`,borderRadius:10,background:form.topMaterial===m?R:"#fff",color:form.topMaterial===m?"#fff":"#1a1a1a",fontSize:13,fontWeight:600,cursor:"pointer"}}>{m}</button>)}
            </div>
          </Field>
          ...
          {needsQuarry&&form.topMaterial&&(
            <div style={{background:"#FFF8F0",border:"1px solid #FBBF24",borderRadius:10,padding:14,marginTop:4}}>
              ...
              <div style={{display:"flex",gap:10,marginBottom:12}}>
                {[{label:"Yes – Select Quarry",val:true},{label:"Skip",val:false}].map(opt=><button key={String(opt.val)} onClick={()=>{set("quarryKnown",opt.val);if(!opt.val)setMany({quarry:"",quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:"",});}} style={{flex:1,padding:10,border:`2px solid ${form.quarryKnown===opt.val?R:"#bbb"}`,borderRadius:8,background:form.quarryKnown===opt.val?R:"#fff",color:form.quarryKnown===opt.val?"#fff":"#1a1a1a",fontWeight:700,cursor:"pointer",fontSize:13}}>{opt.label}</button>)}
              </div>
              {form.quarryKnown===true&&<>
                ...
                {form.quarry&&(qObj||qDB)&&(
                  <div style={{background:"#fff",borderRadius:8,padding:10,marginBottom:10,fontSize:12,color:"#333",border:"1px solid #e5e7eb"}}>
                    ...
                    {(qObj?.notes||qDB?.note)&&<div style={{color:"#666",marginTop:2}}>{qObj?.notes||qDB?.note}</div>}
                  </div>
                )}
                {form.quarry&&qDB&&<Field label="Stone / Material Type">
                  ...
                  {form.quarryMaterialName&&<div style={{display:"flex",gap:8,marginTop:8}}><div style={{flex:1,background:"#EEF2FF",borderRadius:8,padding:10,textAlign:"center"}}><div style={{fontSize:11,color:"#444",fontWeight:700}}>MATERIAL CODE</div><div style={{fontWeight:900,color:NV,fontSize:20}}>#{form.quarryMaterialCode}</div></div><div style={{flex:1,background:"#F0FDF4",borderRadius:8,padding:10,textAlign:"center"}}><div style={{fontSize:11,color:"#444",fontWeight:700}}>GATE RATE / TON</div><div style={{fontWeight:900,color:"#166534",fontSize:20}}>${form.quarryMaterialPrice}</div></div></div>}
                </Field>}
              </>}
            </div>
          )}
        ...
        <Section title="TIME & HOURS" icon="⏱️">
          ...
          <Field label="Lunch Break?">
            <div style={{display:"flex",gap:10}}>
              {[{label:"✓ Worked Through",val:true},{label:"Took 30-min Break",val:false}].map(opt=><button key={String(opt.val)} onClick={()=>set("lunchWorked",opt.val)} style={{flex:1,padding:"9px 4px",border:`2px solid ${form.lunchWorked===opt.val?R:"#bbb"}`,borderRadius:8,background:form.lunchWorked===opt.val?R:"#fff",color:form.lunchWorked===opt.val?"#fff":"#1a1a1a",fontSize:12,fontWeight:700,cursor:"pointer"}}>{opt.label}</button>)}
            </div>
          </Field>
          <div style={{background:"#EEF2FF",borderRadius:10,padding:14,marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div><div style={{fontWeight:800,fontSize:12,color:NV,letterSpacing:0.5}}>STRAIGHT TIME HOURS</div><div style={{fontSize:12,color:"#444",fontWeight:500}}>Auto-calculated · edit if needed</div></div><div style={{fontSize:30,fontWeight:900,color:NV}}>{form.straightTime||"—"}</div></div>
          ...
          <div style={{background:"#FFF7ED",borderRadius:10,padding:14,border:"1px solid #fed7aa"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontWeight:700,fontSize:13,color:"#92400E"}}>Overtime Pay?</div><div style={{fontSize:12,color:"#b45309",fontWeight:500}}>Enable only if this company pays OT</div></div>
        ...
        <Section title="BILLING RATES" icon="💲">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="Rate Per Hour"><div style={{position:"relative"}}><span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#444",fontWeight:700,pointerEvents:"none"}}>$</span><input type="number" value={form.ratePerHour} onChange={e=>set("ratePerHour",e.target.value)} style={{...S.inp,paddingLeft:26}} placeholder="0.00"/></div></Field>
            <Field label="Rate Per Load"><div style={{position:"relative"}}><span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#444",fontWeight:700,pointerEvents:"none"}}>$</span><input type="number" value={form.ratePerLoad} onChange={e=>set("ratePerLoad",e.target.value)} style={{...S.inp,paddingLeft:26}} placeholder="0.00"/></div></Field>
          </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:20}}>
        <div style={{background:"#FFF8F0",border:"1px solid #FBBF24",borderRadius:12,padding:14,marginBottom:20}}><p style={{margin:0,fontSize:14,color:"#92400E",fontWeight:700}}>⚠️ Both Driver and Foreman must sign before submitting.</p></div>
        {[{key:"driver",label:"Driver Signature",sub:form.driver,sig:driverSig,setSig:setDriverSig},{key:"foreman",label:"Foreman Signature",sub:"Hand phone to foreman",sig:foremanSig,setSig:setForemanSig}].map(s=>(
          <div key={s.key} style={{background:"#fff",borderRadius:14,padding:20,marginBottom:16,boxShadow:"0 2px 12px rgba(0,0,0,0.08)"}}>
            ...
            {s.sig&&<button onClick={()=>s.setSig(null)} style={{marginTop:6,background:"none",border:"none",color:R,fontSize:13,cursor:"pointer",fontWeight:600}}>Re-sign</button>}
          </div>
        ))}
    ...
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      <div style={{background:`linear-gradient(135deg,${DR},${R})`,padding:"16px 20px",color:"#fff"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          {[{label:"📤 Share",action:handleShare},{label:"✉️ Email",action:handleEmail},{label:"🖨 Download TXT",action:handlePrint}].map(a=><button key={a.label} onClick={a.action} style={{flex:1,background:"rgba(255,255,255,0.2)",border:"none",color:"#fff",borderRadius:8,padding:"10px 4px",fontSize:12,cursor:"pointer",fontWeight:600}}>{a.label}</button>)}
        </div>
      ...
      <div style={{padding:16}}>
        <div style={{background:"#fff",borderRadius:14,padding:24,boxShadow:"0 4px 24px rgba(0,0,0,0.12)",fontFamily:"Georgia,serif"}}>
          <div style={{textAlign:"center",borderBottom:"3px solid #1a1a1a",paddingBottom:12,marginBottom:14}}>
            ...
            <div style={{fontSize:14,color:"#333",fontWeight:500}}>25603 Bethel Lane • Dow, IL 62022</div>
            <div style={{display:"flex",justifyContent:"space-between",marginTop:10,fontSize:13}}><div><strong>Bob Sanders</strong><br/>618-818-8225</div><div style={{textAlign:"center",color:"#444",fontSize:11,lineHeight:1.7}}>ROCK • SAND • DIRT • ASPHALT<br/>GRAVEL • BUILDING MATERIALS<br/>STRUCTURAL DEMO</div><div style={{textAlign:"right"}}><strong>Glenn Sanders</strong><br/>618-779-6576</div></div>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:12,fontSize:14}}><span><strong>No.</strong> {wo.workOrderNumber}</span><span><strong>Date:</strong> {fmt(wo.date)}</span></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:20}}>
            {[{label:"Driver",sig:wo.driverSig,name:wo.driver},{label:"Foreman",sig:wo.foremanSig,name:"Foreman"}].map(s=><div key={s.label}><div style={{fontSize:12,color:"#444",marginBottom:4,fontWeight:700}}>{s.label} Signature</div><div style={{border:"1px solid #ccc",borderRadius:6,height:70,overflow:"hidden",background:"#fafafa"}}>{s.sig&&<img src={s.sig} alt={s.label} style={{width:"100%",height:"100%",objectFit:"contain"}}/>}</div><div style={{fontSize:12,color:"#333",borderTop:"1px solid #ccc",paddingTop:4,marginTop:4,fontWeight:600}}>{s.name}</div></div>)}
          </div>
          <div style={{marginTop:20,paddingTop:14,borderTop:"2px solid #1a1a1a",fontSize:11,color:"#444",textAlign:"center",lineHeight:1.6}}><strong>EXCLUSIVE USE OF VEHICLE ORDERED AND PAYMENT OF CHARGES GUARANTEED BY SHIPPER</strong><br/>TERMS: net 30 Days. Accounts over 30 days subject to a 2% service charge.</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:14}}>
          <button onClick={onClose} style={{...S.btn,background:"#fff",color:"#333",border:"2px solid #aaa",fontWeight:700}}>View Queue</button>
    ...
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:"16px 20px 80px"}}>
        <div style={{background:`linear-gradient(135deg,${DR},${R})`,borderRadius:16,padding:20,marginBottom:16,color:"#fff"}}>
          <div style={{fontSize:12,opacity:0.8,letterSpacing:1,marginBottom:12}}>TODAY'S HOURS SUMMARY</div>
          <div style={{display:"flex",gap:12}}>
            {[{val:totalST.toFixed(2),label:"Straight Hrs"},{val:totalOT.toFixed(2),label:"Overtime Hrs"},{val:fmtH(totalST+totalOT),label:"Total",h:true}].map(s=><div key={s.label} style={{flex:1,background:s.h?"rgba(255,255,255,0.25)":"rgba(255,255,255,0.15)",borderRadius:10,padding:14,textAlign:"center",border:s.h?"2px solid rgba(255,255,255,0.4)":"none"}}><div style={{fontSize:28,fontWeight:900}}>{s.val}</div><div style={{fontSize:11,opacity:0.85,marginTop:2}}>{s.label}</div></div>)}
          </div>
        </div>
        {todayWOs.length>0&&<div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:16,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}><div style={{fontSize:12,fontWeight:800,color:"#444",letterSpacing:1,marginBottom:10}}>TODAY'S TICKETS ({todayWOs.length})</div>{todayWOs.map((wo,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:i<todayWOs.length-1?"1px solid #f0f0f0":"none"}}><div><div style={{fontWeight:700,fontSize:14}}>{wo.workOrderNumber}</div><div style={{fontSize:13,color:"#444",fontWeight:500}}>{wo.companyWorkedFor} • {wo.truckNumber}</div></div><div style={{textAlign:"right",fontSize:13}}><div style={{fontWeight:700,color:NV}}>{wo.straightTime}h ST</div>{wo.overtimeEnabled&&wo.overtime&&<div style={{color:R,fontWeight:600}}>+{wo.overtime}h OT</div>}</div></div>)}</div>}
        <div style={{background:"#fff",borderRadius:14,padding:20,boxShadow:"0 2px 10px rgba(0,0,0,0.07)",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><span style={{fontSize:28}}>⛽</span><div><div style={{fontWeight:800,fontSize:16}}>Fuel Report</div><div style={{fontSize:13,color:"#444",fontWeight:500}}>Enter gallons pumped today</div></div>{saved&&<span style={{marginLeft:"auto",background:"#dcfce7",color:"#166534",fontSize:12,fontWeight:700,padding:"4px 10px",borderRadius:20}}>✓ Saved</span>}</div>
          ...
          <div style={{marginBottom:14}}>
            ...
            <div style={{position:"relative"}}><input type="number" value={gallons} onChange={e=>{setGallons(e.target.value);setSaved(false);}} style={{...S.inp,fontSize:32,fontWeight:900,textAlign:"center",color:NV,padding:"16px 12px",height:80}} placeholder="0.0" step="0.1" min="0"/><span style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",fontSize:14,color:"#444",fontWeight:700,pointerEvents:"none"}}>gal</span></div>
            <div style={{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}>{["18.7","19.1","25.0","37.4","42.0","50.0"].map(v=><button key={v} onClick={()=>{setGallons(v);setSaved(false);}} style={{padding:"6px 12px",background:gallons===v?NV:"#f0f0f0",color:gallons===v?"#fff":"#1a1a1a",border:"none",borderRadius:8,fontSize:14,fontWeight:700,cursor:"pointer"}}>{v}</button>)}</div>
          </div>
        ...
        {showConfirm&&<div style={{background:"#dcfce7",border:"1px solid #86efac",borderRadius:12,padding:16,textAlign:"center"}}><div style={{fontSize:24,marginBottom:4}}>✅</div><div style={{fontWeight:700,color:"#166534"}}>End of Day Complete!</div><div style={{fontSize:14,color:"#15803d",marginTop:4,fontWeight:500}}>{fmtH(totalST+totalOT)} worked • {gallons} gal fuel logged</div></div>}
      </div>
    </div>
  );
}
    ...
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:20}}>
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          {["all","complete","draft"].map(f=><button key={f} onClick={()=>setFilter(f)} style={{padding:"7px 18px",borderRadius:20,border:"none",background:filter===f?R:"#fff",color:filter===f?"#fff":"#1a1a1a",fontSize:13,fontWeight:700,cursor:"pointer",boxShadow:"0 1px 4px rgba(0,0,0,0.1)"}}>{f.charAt(0).toUpperCase()+f.slice(1)}</button>)}
        </div>
        {filtered.length===0&&<div style={{textAlign:"center",padding:40,color:"#444",fontWeight:600,fontSize:15}}>No work orders found</div>}
