    ...
    <div style={{background:"#fff",borderRadius:12,padding:14,marginBottom:10,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          {badges.length>0&&<div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}>{badges.map((b,i)=><span key={i} style={{background:b.bg||"#f0f0f0",color:b.color||"#555",fontSize:13,padding:"2px 8px",borderRadius:12,fontWeight:600}}>{b.label}</span>)}</div>}
        </div>
...
function ContactEditModal({type,data,onSave,onDelete,onClose}){
  ...
  const SubHead=({icon,title})=><div style={subSectionStyle}><span style={{fontSize:16}}>{icon}</span><span style={{fontWeight:800,fontSize:13,color:"#333",letterSpacing:1,textTransform:"uppercase"}}>{title}</span></div>;
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:800,display:"flex",flexDirection:"column"}}>
      <div style={{background:`linear-gradient(135deg,${DR},${R})`,padding:"14px 16px",color:"#fff",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:13,opacity:0.75}}>{{company:"Company + billing/AP contacts",driver:"Driver info + emergency contact",quarry:"Quarry + AP + dispatch"}[type]}</div>
        </div>
      </div>
      ...
      <div style={{background:"#fff",padding:"14px 16px",display:"flex",gap:10,boxShadow:"0 -4px 16px rgba(0,0,0,0.12)",flexShrink:0}}>
        ...
        <button onClick={()=>{if(!local.name?.trim()){alert("Name is required");return;}onSave(local);}} style={{...S.btn,background:R,color:"#fff",flex:1,fontSize:15}}>
          ✓ Save {labels[type]}
        </button>
      </div>
    </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${DR} 0%,${R} 50%,#1a1a1a 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{textAlign:"center",marginBottom:36}}>
        ...
        <p style={{color:"rgba(255,255,255,0.65)",fontSize:13,margin:0,letterSpacing:2}}>FIELD OPERATIONS • DOW, IL</p>
      </div>
      <div style={{background:"#fff",borderRadius:20,padding:28,width:"100%",maxWidth:380,boxShadow:"0 20px 60px rgba(0,0,0,0.4)"}}>
        ...
        {err&&<p style={{color:R,fontSize:13,margin:"6px 0 0"}}>{err}</p>}
        ...
        <button onClick={onAdmin} style={{marginTop:10,width:"100%",background:"none",border:"none",color:"#555",fontSize:13,cursor:"pointer",padding:8}}>Admin / Settings ›</button>
      </div>
      <p style={{color:"rgba(255,255,255,0.35)",fontSize:13,marginTop:24}}>Bob Sanders 618-818-8225 • Glenn Sanders 618-779-6576</p>
    </div>
  );
}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      <div style={{background:`linear-gradient(135deg,${DR},${R})`,padding:"20px 20px 30px",color:"#fff"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div><div style={{fontFamily:"Georgia,serif",fontWeight:900,fontSize:26}}>RJS HAULING</div><div style={{fontSize:13,opacity:0.8}}>Welcome, {driver.split(" ")[0]}</div></div>
          <button onClick={onLogout} style={{background:"rgba(255,255,255,0.2)",border:"none",color:"#fff",borderRadius:8,padding:"6px 14px",fontSize:13,cursor:"pointer"}}>Sign Out</button>
        </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{display:"flex",background:"#fff",borderBottom:"2px solid #f0f0f0",overflowX:"auto"}}>
        {TABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"12px 4px",border:"none",background:"none",color:tab===t.id?R:"#222",fontWeight:tab===t.id?800:500,fontSize:13,cursor:"pointer",borderBottom:tab===t.id?`3px solid ${R}`:"3px solid transparent",whiteSpace:"nowrap",minWidth:72}}>{t.label}</button>)}
      </div>

      <div style={{padding:"16px 20px 100px"}}>

        {tab==="companies"&&<>
          <p style={{fontSize:13,color:"#444",margin:"0 0 12px"}}>Tap <strong>Edit</strong> to add address, phone, email, text, primary contact, and billing/AP details.</p>
          {localCompanies.map((co,i)=>(
            <ContactListCard key={i} icon="🏢" name={co.name}
              info={[[co.phone,co.email].filter(Boolean).join(" • "),co.address].filter(Boolean)}
              badges={[co.contact?.name&&{label:`👤 ${co.contact.name}`,bg:"#EEF2FF",color:NV},co.billing?.name&&{label:`💳 ${co.billing.name}`,bg:"#F0FDF4",color:"#166534"}].filter(Boolean)}
              onEdit={()=>setEditModal({type:"company",idx:i})}/>
          ))}
        ...
        {tab==="drivers"&&<>
          <p style={{fontSize:13,color:"#444",margin:"0 0 12px"}}>Tap <strong>Edit</strong> to add address, phone, email, text, CDL #, and emergency contact.</p>
          {localDrivers.map((d,i)=>(
            <ContactListCard key={i} icon="👷" name={d.name}
              info={[[d.phone,d.email].filter(Boolean).join(" • "),d.address,d.emergency?.name?`🚨 Emergency: ${d.emergency.name}${d.emergency.phone?" — "+d.emergency.phone:""}`:null].filter(Boolean)}
              badges={[d.license&&{label:`🪪 ${d.license}`,bg:"#FFF7ED",color:"#92400E"}].filter(Boolean)}
              onEdit={()=>setEditModal({type:"driver",idx:i})}/>
          ))}
        ...
        {tab==="quarries"&&<>
          <p style={{fontSize:13,color:"#444",margin:"0 0 12px"}}>Tap <strong>Edit</strong> to add main office, accounts payable, and scale/dispatch contacts.</p>
          {localQuarries.map((q,i)=>(
            <ContactListCard key={i} icon="🪨" name={q.name}
              info={[[q.phone,q.email].filter(Boolean).join(" • "),q.address].filter(Boolean)}
              badges={[q.ap?.name&&{label:`💳 AP: ${q.ap.name}`,bg:"#F0FDF4",color:"#166534"},q.dispatch?.name&&{label:`⚖️ ${q.dispatch.name}`,bg:"#EEF2FF",color:NV}].filter(Boolean)}
              onEdit={()=>setEditModal({type:"quarry",idx:i})}/>
          ))}
        ...
        {tab==="reports"&&<>
          <div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}>
            <p style={{margin:"0 0 10px",fontWeight:800,fontSize:13,color:"#444",letterSpacing:1}}>TIME PERIOD</p>
            <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
              {[["daily","Day"],["weekly","Week"],["monthly","Month"],["yearly","Year"]].map(([v,l])=><button key={v} onClick={()=>setReportView(v)} style={{padding:"7px 14px",borderRadius:20,border:"none",background:reportView===v?R:"#f0f0f0",color:reportView===v?"#fff":"#333",fontWeight:600,fontSize:13,cursor:"pointer"}}>{l}</button>)}
            </div>
            <select value={reportDriver} onChange={e=>setReportDriver(e.target.value)} style={S.sel}>
              <option value="all">All Drivers</option>
              {localDrivers.map(d=><option key={getName(d)} value={getName(d)}>{getName(d)}</option>)}
            </select>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:14}}>
            {[{label:"Tickets",val:reportWOs.length,color:DR},{label:"Straight Hrs",val:totalST.toFixed(1),color:NV},{label:"OT Hrs",val:totalOT.toFixed(1),color:R},{label:"Gallons",val:reportWOs.reduce((a,w)=>a+parseFloat(w.gallons||0),0).toFixed(0),color:"#92400E"}].map(s=><div key={s.label} style={{background:"#fff",borderRadius:12,padding:14,textAlign:"center",boxShadow:"0 2px 10px rgba(0,0,0,0.08)"}}><div style={{fontSize:22,fontWeight:900,color:s.color}}>{s.val}</div><div style={{fontSize:13,color:"#444",marginTop:2}}>{s.label}</div></div>)}
          </div>
          {Object.keys(byDriver).length>0&&<div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 2px 10px rgba(0,0,0,0.08)"}}>
            <h3 style={{margin:"0 0 12px",fontSize:13,color:"#333",fontWeight:800,letterSpacing:1}}>DRIVER BREAKDOWN</h3>
            {Object.entries(byDriver).sort((a,b)=>b[1].st-a[1].st).map(([d,data])=><div key={d} style={{padding:"14px 0",borderBottom:"1px solid #f0f0f0"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}><div style={{fontWeight:800,fontSize:15}}>{d}</div><div style={{textAlign:"right"}}><div style={{fontWeight:700,color:NV,fontSize:15}}>{data.st.toFixed(2)}h ST</div>{data.ot>0&&<div style={{color:R,fontSize:13,fontWeight:600}}>{data.ot.toFixed(2)}h OT</div>}</div></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><span style={{background:"#EEF2FF",color:NV,fontSize:13,fontWeight:700,padding:"3px 8px",borderRadius:6}}>📋 {data.count} tickets</span>{data.gallons>0&&<span style={{background:"#FFF7ED",color:"#92400E",fontSize:13,fontWeight:700,padding:"3px 8px",borderRadius:6}}>⛽ {data.gallons.toFixed(0)} gal</span>}</div></div>)}
          </div>}
          ...
          <div style={{background:"#fff",borderRadius:14,padding:16,marginTop:16,border:"2px dashed #d1d5db"}}>
            <p style={{margin:"0 0 6px",fontWeight:800,fontSize:13,color:"#444",letterSpacing:1}}>🧪 DEMO / SAMPLE DATA</p>
            <p style={{margin:"0 0 12px",fontSize:13,color:"#444"}}>Load 20 sample work orders to preview reports. Clear when going live.</p>
            ...
            {workOrders.filter(w=>w.isSampleData).length>0&&<p style={{margin:"10px 0 0",fontSize:13,color:"#059669",textAlign:"center"}}>✓ {workOrders.filter(w=>w.isSampleData).length} sample tickets loaded</p>}
          </div>
        </>}
      </div>

      {editModal&&(
        <ContactEditModal type={editModal.type} data={modalData} onSave={handleContactSave} onDelete={editModal.idx!==null?handleContactDelete:undefined} onClose={()=>setEditModal(null)}/>
      )}
    </div>
  );
}
...
function WorkOrderForm({driver,quarries,companies,savedLocations=[],onSubmit,onCancel}){
  const [form,setForm]=useState({
    workOrderNumber:nextWoNum(),date:nowDate(),equipmentType:"Dump",truckNumber:"",
    jobName:"",jobNumber:"",topMaterial:"",materialHauled:"",
    quarryKnown:null,quarry:"",quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:"",
    arrivedTime:"",releasedTime:"",lunchWorked:null,overtimeEnabled:false,straightTime:"",overtime:"",
    fromLocation:"",toLocation:"",
    companyWorkedFor:"",companyAddress:"",companyPhone:"",
    companyContactName:"",companyContactPhone:"",companyContactEmail:"",
    companyBillingName:"",companyBillingPhone:"",companyBillingEmail:"",
    ratePerHour:"",ratePerLoad:"",gallons:"",remarks:"",driver,
  });
  ...
  const handleCompanyPick=useCallback(name=>{
    const co=companies.map(normCompany).find(c=>c.name===name);
    if(co){setMany({companyWorkedFor:co.name,companyAddress:co.address||"",companyPhone:co.phone||"",companyContactName:co.contact?.name||"",companyContactPhone:co.contact?.phone||"",companyContactEmail:co.contact?.email||"",companyBillingName:co.billing?.name||"",companyBillingPhone:co.billing?.phone||"",companyBillingEmail:co.billing?.email||""});}
    else setMany({companyWorkedFor:name});
  },[companies,setMany]);
    ...
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:"16px 20px 100px"}}>
        ...
        <Section title="CUSTOMER / COMPANY" icon="🏢">
          ...
          {(form.companyContactName||form.companyBillingName)&&(
            <div style={{background:"#EEF2FF",borderRadius:8,padding:10,marginBottom:12,fontSize:13}}>
              {form.companyContactName&&<div>👤 <strong>{form.companyContactName}</strong>{form.companyContactPhone?" — "+form.companyContactPhone:""}{form.companyContactEmail?" | "+form.companyContactEmail:""}</div>}
              {form.companyBillingName&&<div style={{marginTop:4}}>💳 Billing: <strong>{form.companyBillingName}</strong>{form.companyBillingPhone?" — "+form.companyBillingPhone:""}</div>}
            </div>
          )}
        ...
        <Section title="MATERIAL HAULED" icon="⛏️">
          <Field label="Material Category" err={errors.topMaterial}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:12}}>
              {TOP_MATERIALS.map(m=><button key={m} onClick={()=>setMany({topMaterial:m,materialHauled:"",quarry:"",quarryKnown:null,quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:""})} style={{padding:"10px 8px",border:`2px solid ${form.topMaterial===m?R:"#ddd"}`,borderRadius:10,background:form.topMaterial===m?R:"#fff",color:form.topMaterial===m?"#fff":"#333",fontSize:13,fontWeight:600,cursor:"pointer"}}>{m}</button>)}
            </div>
          </Field>
          ...
          {needsQuarry&&form.topMaterial&&(
            <div style={{background:"#FFF8F0",border:"1px solid #FBBF24",borderRadius:10,padding:14,marginTop:4}}>
              <p style={{margin:"0 0 10px",fontWeight:700,color:"#92400E",fontSize:13}}>🪨 Do you know the quarry / source?</p>
              <div style={{display:"flex",gap:10,marginBottom:12}}>
                {[{label:"Yes – Select Quarry",val:true},{label:"Skip",val:false}].map(opt=><button key={String(opt.val)} onClick={()=>{set("quarryKnown",opt.val);if(!opt.val)setMany({quarry:"",quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:"",});}} style={{flex:1,padding:10,border:`2px solid ${form.quarryKnown===opt.val?R:"#ddd"}`,borderRadius:8,background:form.quarryKnown===opt.val?R:"#fff",color:form.quarryKnown===opt.val?"#fff":"#333",fontWeight:600,cursor:"pointer",fontSize:13}}>{opt.label}</button>)}
              </div>
              {form.quarryKnown===true&&<>
                ...
                {form.quarry&&(qObj||qDB)&&(
                  <div style={{background:"#fff",borderRadius:8,padding:10,marginBottom:10,fontSize:13,color:"#333",border:"1px solid #e5e7eb"}}>
                    <div style={{fontWeight:700,color:"#1a1a1a"}}>{form.quarry}</div>
                    {(qObj?.address||qDB?.address)&&<div>{qObj?.address||qDB?.address}</div>}
                    <div style={{display:"flex",gap:12,marginTop:2,flexWrap:"wrap"}}>
                      {(qObj?.phone||qDB?.phone)&&<span>📞 {qObj?.phone||qDB?.phone}</span>}
                      {(qObj?.email||qDB?.email)&&<span>✉️ {qObj?.email||qDB?.email}</span>}
                      {qObj?.text&&<span>💬 {qObj.text}</span>}
                    </div>
                    {qObj?.ap?.name&&<div style={{marginTop:4,color:"#166534"}}>💳 AP: {qObj.ap.name}{qObj.ap.phone?" — "+qObj.ap.phone:""}</div>}
                    {qObj?.dispatch?.name&&<div style={{color:NV}}>⚖️ {qObj.dispatch.name}{qObj.dispatch.phone?" — "+qObj.dispatch.phone:""}</div>}
                    {(qObj?.notes||qDB?.note)&&<div style={{color:"#555",marginTop:2}}>{qObj?.notes||qDB?.note}</div>}
                  </div>
                )}
                {form.quarry&&qDB&&<Field label="Stone / Material Type">
                  ...
                  {form.quarryMaterialName&&<div style={{display:"flex",gap:8,marginTop:8}}><div style={{flex:1,background:"#EEF2FF",borderRadius:8,padding:10,textAlign:"center"}}><div style={{fontSize:10,color:"#444"}}>MATERIAL CODE</div><div style={{fontWeight:900,color:NV,fontSize:20}}>#{form.quarryMaterialCode}</div></div><div style={{flex:1,background:"#F0FDF4",borderRadius:8,padding:10,textAlign:"center"}}><div style={{fontSize:10,color:"#444"}}>GATE RATE / TON</div><div style={{fontWeight:900,color:"#166534",fontSize:20}}>${form.quarryMaterialPrice}</div></div></div>}
                </Field>}
              </>}
            </div>
          )}
        ...
        <Section title="TIME & HOURS" icon="⏱️">
          ...
          <Field label="Lunch Break?">
            <div style={{display:"flex",gap:10}}>
              {[{label:"✓ Worked Through",val:true},{label:"Took 30-min Break",val:false}].map(opt=><button key={String(opt.val)} onClick={()=>set("lunchWorked",opt.val)} style={{flex:1,padding:"9px 4px",border:`2px solid ${form.lunchWorked===opt.val?R:"#ddd"}`,borderRadius:8,background:form.lunchWorked===opt.val?R:"#fff",color:form.lunchWorked===opt.val?"#fff":"#333",fontSize:13,fontWeight:600,cursor:"pointer"}}>{opt.label}</button>)}
            </div>
          </Field>
          <div style={{background:"#EEF2FF",borderRadius:10,padding:14,marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div><div style={{fontWeight:800,fontSize:13,color:NV,letterSpacing:0.5}}>STRAIGHT TIME HOURS</div><div style={{fontSize:13,color:"#444"}}>Auto-calculated · edit if needed</div></div><div style={{fontSize:30,fontWeight:900,color:NV}}>{form.straightTime||"—"}</div></div>
          ...
          <div style={{background:"#FFF7ED",borderRadius:10,padding:14,border:"1px solid #fed7aa"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontWeight:700,fontSize:13,color:"#92400E"}}>Overtime Pay?</div><div style={{fontSize:13,color:"#b45309"}}>Enable only if this company pays OT</div></div>
        ...
        <Section title="BILLING RATES" icon="💲">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="Rate Per Hour"><div style={{position:"relative"}}><span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#444",pointerEvents:"none"}}>$</span><input type="number" value={form.ratePerHour} onChange={e=>set("ratePerHour",e.target.value)} style={{...S.inp,paddingLeft:26}} placeholder="0.00"/></div></Field>
            <Field label="Rate Per Load"><div style={{position:"relative"}}><span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#444",pointerEvents:"none"}}>$</span><input type="number" value={form.ratePerLoad} onChange={e=>set("ratePerLoad",e.target.value)} style={{...S.inp,paddingLeft:26}} placeholder="0.00"/></div></Field>
          </div>
      ...
      <div style={{padding:20}}>
        ...
        {[{key:"driver",label:"Driver Signature",sub:form.driver,sig:driverSig,setSig:setDriverSig},{key:"foreman",label:"Foreman Signature",sub:"Hand phone to foreman",sig:foremanSig,setSig:setForemanSig}].map(s=>(
          <div key={s.key} style={{background:"#fff",borderRadius:14,padding:20,marginBottom:16,boxShadow:"0 2px 12px rgba(0,0,0,0.08)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div><div style={{fontWeight:700,fontSize:15}}>{s.label}</div><div style={{color:"#444",fontSize:13}}>{s.sub}</div></div><span style={{background:s.sig?"#dcfce7":"#fee2e2",color:s.sig?"#166534":"#991b1b",fontSize:13,padding:"3px 10px",borderRadius:20,fontWeight:600}}>{s.sig?"✓ Signed":"Pending"}</span></div>
            ...
            {s.sig&&<button onClick={()=>s.setSig(null)} style={{marginTop:6,background:"none",border:"none",color:R,fontSize:13,cursor:"pointer"}}>Re-sign</button>}
          </div>
        ))}
...
function InvoicePreview({workOrder:wo,onClose,onNewOrder}){
  ...
  const buildText=()=>[
    "=================================","          RJS HAULING","  25603 Bethel Lane • Dow, IL 62022","  Bob: 618-818-8225 | Glenn: 618-779-6576","=================================",
    `WO #: ${wo.workOrderNumber}`,`Date: ${fmt(wo.date)}`,`Driver: ${wo.driver}`,`Truck: ${wo.truckNumber} (${wo.equipmentType})`,
    "---",`Job: ${wo.jobName||"—"} / ${wo.jobNumber||"—"}`,`Company: ${wo.companyWorkedFor||"—"}`,
    wo.companyAddress&&`Address: ${wo.companyAddress}`,wo.companyPhone&&`Phone: ${wo.companyPhone}`,
    wo.companyContactName&&`Contact: ${wo.companyContactName}${wo.companyContactPhone?" — "+wo.companyContactPhone:""}`,
    wo.companyBillingName&&`Billing: ${wo.companyBillingName}${wo.companyBillingPhone?" — "+wo.companyBillingPhone:""}`,
    "---",`Material: ${wo.topMaterial}${wo.materialHauled?" › "+wo.materialHauled:""}`,
    wo.quarry&&`Quarry: ${wo.quarry}`,wo.quarryMaterialName&&`Stone: ${wo.quarryMaterialName} #${wo.quarryMaterialCode} @ $${wo.quarryMaterialPrice}/ton`,
    `From: ${wo.fromLocation||"—"}`,`To: ${wo.toLocation||"—"}`,
    "---",`Arrived: ${wo.arrivedTime||"—"}`,`Released: ${wo.releasedTime||"—"}`,
    wo.lunchWorked!==null&&`Lunch: ${wo.lunchWorked?"Worked through":"Took 30-min break"}`,
    `Straight Hrs: ${wo.straightTime||"—"}`,wo.overtimeEnabled&&wo.overtime&&`Overtime: ${wo.overtime} hrs`,
    "---",wo.ratePerHour&&`Rate/Hr: $${wo.ratePerHour}`,wo.ratePerLoad&&`Rate/Load: $${wo.ratePerLoad}`,wo.gallons&&`Fuel: ${wo.gallons} gal`,wo.remarks&&`Notes: ${wo.remarks}`,
    "---","Signed by Driver & Foreman","=================================","TERMS: Net 30. Over 30 days = 2% service charge.","=================================",
  ].filter(Boolean).join("\n");
  ...
  const InvRow=({label,value})=>value?<div style={{display:"flex",borderBottom:"1px solid #eee",padding:"8px 0",fontSize:14}}><div style={{width:130,color:"#444",flexShrink:0,fontStyle:"italic",fontWeight:600}}>{label}</div><div style={{fontWeight:700,color:"#111"}}>{value}</div></div>:null;
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      <div style={{background:`linear-gradient(135deg,${DR},${R})`,padding:"16px 20px",color:"#fff"}}>
        ...
        <div style={{fontSize:13,opacity:0.8,marginBottom:12}}>{wo.workOrderNumber} • Review &amp; share</div>
        <div style={{display:"flex",gap:8}}>
          {[{label:"📤 Share",action:handleShare},{label:"✉️ Email",action:handleEmail},{label:"🖨 Download TXT",action:handlePrint}].map(a=><button key={a.label} onClick={a.action} style={{flex:1,background:"rgba(255,255,255,0.2)",border:"none",color:"#fff",borderRadius:8,padding:"10px 4px",fontSize:13,cursor:"pointer",fontWeight:600}}>{a.label}</button>)}
        </div>
        {msg&&<div style={{marginTop:10,background:"rgba(255,255,255,0.15)",borderRadius:8,padding:"8px 12px",fontSize:13,fontWeight:600}}>{msg}</div>}
      </div>
      <div style={{padding:16}}>
        <div style={{background:"#fff",borderRadius:14,padding:24,boxShadow:"0 4px 24px rgba(0,0,0,0.12)",fontFamily:"Georgia,serif"}}>
          <div style={{textAlign:"center",borderBottom:"3px solid #1a1a1a",paddingBottom:12,marginBottom:14}}>
            ...
            <div style={{fontSize:14,color:"#333"}}>25603 Bethel Lane • Dow, IL 62022</div>
            <div style={{display:"flex",justifyContent:"space-between",marginTop:10,fontSize:13}}><div><strong>Bob Sanders</strong><br/>618-818-8225</div><div style={{textAlign:"center",color:"#444",fontSize:13,lineHeight:1.7}}>ROCK • SAND • DIRT • ASPHALT<br/>GRAVEL • BUILDING MATERIALS<br/>STRUCTURAL DEMO</div><div style={{textAlign:"right"}}><strong>Glenn Sanders</strong><br/>618-779-6576</div></div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:20}}>
            {[{label:"Driver",sig:wo.driverSig,name:wo.driver},{label:"Foreman",sig:wo.foremanSig,name:"Foreman"}].map(s=><div key={s.label}><div style={{fontSize:13,color:"#444",marginBottom:4,fontWeight:700}}>{s.label} Signature</div><div style={{border:"1px solid #ccc",borderRadius:6,height:70,overflow:"hidden",background:"#fafafa"}}>{s.sig&&<img src={s.sig} alt={s.label} style={{width:"100%",height:"100%",objectFit:"contain"}}/>}</div><div style={{fontSize:13,color:"#333",borderTop:"1px solid #ccc",paddingTop:4,marginTop:4,fontWeight:600}}>{s.name}</div></div>)}
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:14}}>
          <button onClick={onClose} style={{...S.btn,background:"#fff",color:"#333",border:"1px solid #ddd"}}>View Queue</button>
    ...
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:"16px 20px 80px"}}>
        <div style={{background:`linear-gradient(135deg,${DR},${R})`,borderRadius:16,padding:20,marginBottom:16,color:"#fff"}}>
          <div style={{fontSize:13,opacity:0.8,letterSpacing:1,marginBottom:12}}>TODAY'S HOURS SUMMARY</div>
          <div style={{display:"flex",gap:12}}>
            {[{val:totalST.toFixed(2),label:"Straight Hrs"},{val:totalOT.toFixed(2),label:"Overtime Hrs"},{val:fmtH(totalST+totalOT),label:"Total",h:true}].map(s=><div key={s.label} style={{flex:1,background:s.h?"rgba(255,255,255,0.25)":"rgba(255,255,255,0.15)",borderRadius:10,padding:14,textAlign:"center",border:s.h?"2px solid rgba(255,255,255,0.4)":"none"}}><div style={{fontSize:28,fontWeight:900}}>{s.val}</div><div style={{fontSize:13,opacity:0.85,marginTop:2}}>{s.label}</div></div>)}
          </div>
        </div>
        {todayWOs.length>0&&<div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:16,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}><div style={{fontSize:13,fontWeight:800,color:"#444",letterSpacing:1,marginBottom:10}}>TODAY'S TICKETS ({todayWOs.length})</div>{todayWOs.map((wo,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:i<todayWOs.length-1?"1px solid #f0f0f0":"none"}}><div><div style={{fontWeight:600,fontSize:13}}>{wo.workOrderNumber}</div><div style={{fontSize:13,color:"#444"}}>{wo.companyWorkedFor} • {wo.truckNumber}</div></div><div style={{textAlign:"right",fontSize:13}}><div style={{fontWeight:700,color:NV}}>{wo.straightTime}h ST</div>{wo.overtimeEnabled&&wo.overtime&&<div style={{color:R}}>+{wo.overtime}h OT</div>}</div></div>)}</div>}
        <div style={{background:"#fff",borderRadius:14,padding:20,boxShadow:"0 2px 10px rgba(0,0,0,0.07)",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><span style={{fontSize:28}}>⛽</span><div><div style={{fontWeight:800,fontSize:16}}>Fuel Report</div><div style={{fontSize:13,color:"#444"}}>Enter gallons pumped today</div></div>{saved&&<span style={{marginLeft:"auto",background:"#dcfce7",color:"#166534",fontSize:13,fontWeight:700,padding:"4px 10px",borderRadius:20}}>✓ Saved</span>}</div>
          ...
          <div style={{marginBottom:14}}>
            ...
            <div style={{position:"relative"}}><input type="number" value={gallons} onChange={e=>{setGallons(e.target.value);setSaved(false);}} style={{...S.inp,fontSize:32,fontWeight:900,textAlign:"center",color:NV,padding:"16px 12px",height:80}} placeholder="0.0" step="0.1" min="0"/><span style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",fontSize:14,color:"#444",fontWeight:600,pointerEvents:"none"}}>gal</span></div>
            <div style={{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}>{["18.7","19.1","25.0","37.4","42.0","50.0"].map(v=><button key={v} onClick={()=>{setGallons(v);setSaved(false);}} style={{padding:"6px 12px",background:gallons===v?NV:"#f0f0f0",color:gallons===v?"#fff":"#333",border:"none",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer"}}>{v}</button>)}</div>
          </div>
    ...
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:20}}>
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          {["all","complete","draft"].map(f=><button key={f} onClick={()=>setFilter(f)} style={{padding:"6px 16px",borderRadius:20,border:"none",background:filter===f?R:"#fff",color:filter===f?"#fff":"#333",fontSize:13,fontWeight:600,cursor:"pointer",boxShadow:"0 1px 4px rgba(0,0,0,0.1)"}}>{f.charAt(0).toUpperCase()+f.slice(1)}</button>)}
        </div>
        {filtered.length===0&&<div style={{textAlign:"center",padding:40,color:"#444",fontWeight:600}}>No work orders found</div>}
        {filtered.map((wo,i)=><div key={i} onClick={()=>onView(wo)} style={{background:"#fff",borderRadius:14,padding:16,marginBottom:12,boxShadow:"0 2px 10px rgba(0,0,0,0.08)",cursor:"pointer"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div><div style={{fontWeight:700,fontSize:15,fontFamily:"Georgia,serif",color:DR}}>{wo.workOrderNumber}</div><div style={{color:"#444",fontSize:13,marginTop:2,fontWeight:500}}>{fmt(wo.date)} • {wo.driver}</div><div style={{fontSize:14,marginTop:4,fontWeight:700,color:"#1a1a1a"}}>{wo.companyWorkedFor||"—"}</div><div style={{color:"#444",fontSize:13,fontWeight:500}}>{wo.truckNumber} • {wo.topMaterial}</div></div><div style={{textAlign:"right"}}><span style={{background:wo.status==="complete"?"#dcfce7":"#fef9c3",color:wo.status==="complete"?"#166534":"#854d0e",fontSize:13,padding:"3px 10px",borderRadius:20,fontWeight:700}}>{wo.status==="complete"?"✓ Signed":"Draft"}</span>{wo.straightTime&&<div style={{fontSize:13,color:"#333",marginTop:8,fontWeight:600}}>{wo.straightTime}h{wo.overtimeEnabled&&wo.overtime?" + "+wo.overtime+"h OT":""}</div>}</div></div></div>)}
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  const handleAdminSave=async({drivers:d,companies:c,quarries:q})=>{
    setDrivers(d);setCompanies(c);setQuarries(q);
    try{await window.storage.set("rjs-drivers",JSON.stringify(d));await window.storage.set("rjs-companies",JSON.stringify(c));await window.storage.set("rjs-quarries",JSON.stringify(q));}catch(e){}
  };
  ...
  const handleSigComplete=async(driverSig,foremanSig)=>{
    if(pendingForm.fromLocation)addLocation(pendingForm.fromLocation);
    if(pendingForm.toLocation)addLocation(pendingForm.toLocation);
    const wo={...pendingForm,driverSig,foremanSig,status:"complete",submittedAt:Date.now()};
    await saveWOs([...workOrders,wo]);setViewingWO(wo);setScreen("invoice");
  };
