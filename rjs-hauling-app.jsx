...
// ── STYLES — dark text for outdoor daylight readability ───────────────────────
const S={
    ...
    <div style={{background:"#fff",borderRadius:12,padding:14,marginBottom:10,boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><span>{icon}</span><span style={{fontWeight:800,fontSize:15,color:"#1a1a1a"}}>{name||"—"}</span></div>
          {info.filter(Boolean).map((line,i)=><div key={i} style={{fontSize:13,color:"#333",marginBottom:2,wordBreak:"break-all"}}>{line}</div>)}
          {badges.length>0&&<div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}>{badges.map((b,i)=><span key={i} style={{background:b.bg||"#f0f0f0",color:b.color||"#444",fontSize:13,padding:"2px 8px",borderRadius:12,fontWeight:700}}>{b.label}</span>)}</div>}
        </div>
        <button onClick={onEdit} style={{...S.btn,background:"#e8e8e8",color:"#1a1a1a",padding:"8px 14px",fontSize:13,marginLeft:12,flexShrink:0,fontWeight:700}}>Edit</button>
      </div>
    </div>
  );
}
...
function ContactEditModal({type,data,onSave,onDelete,onClose}){
  ...
  const SubHead=({icon,title})=><div style={subSectionStyle}><span style={{fontSize:16}}>{icon}</span><span style={{fontWeight:800,fontSize:13,color:"#222",letterSpacing:1,textTransform:"uppercase"}}>{title}</span></div>;
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:800,display:"flex",flexDirection:"column"}}>
      <div style={{background:`linear-gradient(135deg,${DR},${R})`,padding:"14px 16px",color:"#fff",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:13,opacity:0.8}}>{{company:"Company + billing/AP contacts",driver:"Driver info + emergency contact",quarry:"Quarry + AP + dispatch"}[type]}</div>
        </div>
      </div>
      ...
      <div style={{background:"#fff",padding:"14px 16px",display:"flex",gap:10,boxShadow:"0 -4px 16px rgba(0,0,0,0.12)",flexShrink:0}}>
        ...
        <button onClick={onClose} style={{...S.btn,background:"#e8e8e8",color:"#1a1a1a",padding:"12px 16px"}}>Cancel</button>
...
function LocationInput({value,onChange,placeholder,savedLocations}){
  ...
  return <div style={{position:"relative"}}><input value={value} onChange={e=>{onChange(e.target.value);setOpen(true);}} onFocus={()=>setOpen(true)} onBlur={()=>setTimeout(()=>setOpen(false),180)} style={S.inp} placeholder={placeholder} autoComplete="off" />{open&&matches.length>0&&<div style={{position:"absolute",top:"100%",left:0,right:0,background:"#fff",border:"2px solid #aaa",borderRadius:8,boxShadow:"0 4px 16px rgba(0,0,0,0.15)",zIndex:200,maxHeight:160,overflowY:"auto"}}>{matches.slice(0,6).map(m=><div key={m} onMouseDown={()=>{onChange(m);setOpen(false);}} style={{padding:"10px 14px",fontSize:14,color:"#111",fontWeight:500,cursor:"pointer",borderBottom:"1px solid #eee"}} onMouseEnter={e=>e.target.style.background="#f0f0f0"} onMouseLeave={e=>e.target.style.background="#fff"}>📍 {m}</div>)}</div>}</div>;
}
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${DR} 0%,${R} 50%,#1a1a1a 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{textAlign:"center",marginBottom:36}}>
        <div style={{display:"inline-block",background:"#fff",borderRadius:16,padding:"18px 32px",marginBottom:12}}>
          ...
          <div style={{fontFamily:"Georgia,serif",fontWeight:700,fontSize:18,color:"#111",letterSpacing:4}}>HAULING</div>
        </div>
        <p style={{color:"rgba(255,255,255,0.75)",fontSize:13,margin:0,letterSpacing:2}}>FIELD OPERATIONS • DOW, IL</p>
      </div>
      <div style={{background:"#fff",borderRadius:20,padding:28,width:"100%",maxWidth:380,boxShadow:"0 20px 60px rgba(0,0,0,0.4)"}}>
        ...
        <p style={{color:"#333",fontSize:14,margin:"0 0 20px"}}>Select your name to begin</p>
        ...
        {err&&<p style={{color:R,fontSize:13,margin:"6px 0 0",fontWeight:700}}>{err}</p>}
        ...
        <button onClick={onAdmin} style={{marginTop:10,width:"100%",background:"none",border:"none",color:"#444",fontSize:13,cursor:"pointer",padding:8,fontWeight:600}}>Admin / Settings ›</button>
      </div>
      <p style={{color:"rgba(255,255,255,0.5)",fontSize:13,marginTop:24,fontWeight:500}}>Bob Sanders 618-818-8225 • Glenn Sanders 618-779-6576</p>
    </div>
  );
}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      <div style={{background:`linear-gradient(135deg,${DR},${R})`,padding:"20px 20px 30px",color:"#fff"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div><div style={{fontFamily:"Georgia,serif",fontWeight:900,fontSize:26}}>RJS HAULING</div><div style={{fontSize:13,opacity:0.85}}>Welcome, {driver.split(" ")[0]}</div></div>
          <button onClick={onLogout} style={{background:"rgba(255,255,255,0.25)",border:"none",color:"#fff",borderRadius:8,padding:"6px 14px",fontSize:13,cursor:"pointer",fontWeight:700}}>Sign Out</button>
        </div>
        <div style={{display:"flex",gap:10,marginTop:18}}>
          {[{label:"Today",val:todayWOs.length},{label:"This Week",val:weekWOs.length},{label:"All Time",val:workOrders.length}].map(s=><div key={s.label} style={{flex:1,background:"rgba(255,255,255,0.18)",borderRadius:10,padding:"12px 10px",textAlign:"center"}}><div style={{fontSize:26,fontWeight:900}}>{s.val}</div><div style={{fontSize:11,opacity:0.9,marginTop:2,fontWeight:600}}>{s.label}</div></div>)}
        </div>
      </div>
      <div style={{padding:20,marginTop:-10}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16}}>
          {[{icon:"📋",label:"Queue",sub:`${workOrders.length}`,action:onQueue},{icon:"📊",label:"Reports",sub:"Daily/Wkly",action:onReports},{icon:"⚙️",label:"Admin",sub:"Contacts",action:onAdmin}].map(a=><button key={a.label} onClick={a.action} style={{background:"#fff",border:"none",borderRadius:14,padding:"16px 10px",cursor:"pointer",textAlign:"center",boxShadow:"0 2px 10px rgba(0,0,0,0.08)"}}><div style={{fontSize:24,marginBottom:4}}>{a.icon}</div><div style={{fontWeight:800,color:"#1a1a1a",fontSize:13}}>{a.label}</div><div style={{color:"#444",fontSize:13,fontWeight:600,marginTop:2}}>{a.sub}</div></button>)}
        </div>
        {todayWOs.length>0&&<div style={{background:"#fff",borderRadius:14,padding:16,boxShadow:"0 2px 12px rgba(0,0,0,0.08)"}}><h3 style={{margin:"0 0 12px",fontSize:13,color:"#222",fontWeight:800,letterSpacing:1}}>TODAY'S TICKETS</h3>{todayWOs.slice(-5).reverse().map((wo,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:i<Math.min(todayWOs.length,5)-1?"1px solid #f0f0f0":"none"}}><div><div style={{fontWeight:700,fontSize:14,color:"#1a1a1a"}}>{wo.workOrderNumber}</div><div style={{color:"#333",fontSize:13,fontWeight:600}}>{wo.companyWorkedFor||"—"} • {wo.topMaterial}</div></div><span style={{background:wo.status==="complete"?"#dcfce7":"#fef9c3",color:wo.status==="complete"?"#166534":"#854d0e",fontSize:13,padding:"3px 10px",borderRadius:20,fontWeight:700}}>{wo.status==="complete"?"✓ Signed":"Draft"}</span></div>)}</div>}
      </div>
    </div>
  );
}
...
function AdminScreen({onBack,drivers,companies,quarries,workOrders,fuelLogs=[],onSave,onLoadSample,onClearSample}){
  ...
  const handleContactSave=saved=>{const{type,idx}=editModal;if(type==="driver"){const u=[...localDrivers];if(idx!==null)u[idx]=saved;else u.push(saved);setLocalDrivers(u);autoSave(u,null,null);}else if(type==="company"){const u=[...localCompanies];if(idx!==null)u[idx]=saved;else u.push(saved);setLocalCompanies(u);autoSave(null,u,null);}else{const u=[...localQuarries];if(idx!==null)u[idx]=saved;else u.push(saved);setLocalQuarries(u);autoSave(null,null,u);}setEditModal(null);};
  const handleContactDelete=()=>{const{type,idx}=editModal;if(type==="driver"){const u=localDrivers.filter((_,i)=>i!==idx);setLocalDrivers(u);autoSave(u,null,null);}else if(type==="company"){const u=localCompanies.filter((_,i)=>i!==idx);setLocalCompanies(u);autoSave(null,u,null);}else{const u=localQuarries.filter((_,i)=>i!==idx);setLocalQuarries(u);autoSave(null,null,u);}setEditModal(null);};
  const now=new Date();
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{display:"flex",background:"#fff",borderBottom:"2px solid #e0e0e0",overflowX:"auto"}}>
        {TABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"12px 4px",border:"none",background:"none",color:tab===t.id?R:"#222",fontWeight:tab===t.id?800:600,fontSize:13,cursor:"pointer",borderBottom:tab===t.id?`3px solid ${R}`:"3px solid transparent",whiteSpace:"nowrap",minWidth:72}}>{t.label}</button>)}
      </div>
      <div style={{padding:"16px 20px 100px"}}>
        {tab==="companies"&&<>
          <p style={{fontSize:13,color:"#333",margin:"0 0 12px",fontWeight:600}}>Tap <strong>Edit</strong> to add address, phone, email, text, primary contact, and billing/AP details.</p>
        ...
        {tab==="drivers"&&<>
          <p style={{fontSize:13,color:"#333",margin:"0 0 12px",fontWeight:600}}>Tap <strong>Edit</strong> to add address, phone, email, text, CDL #, and emergency contact.</p>
        ...
        {tab==="quarries"&&<>
          <p style={{fontSize:13,color:"#333",margin:"0 0 12px",fontWeight:600}}>Tap <strong>Edit</strong> to add main office, accounts payable, and scale/dispatch contacts.</p>
        ...
        {tab==="reports"&&<>
          ...
          {exportMsg&&<div style={{marginTop:10,background:"#EEF2FF",borderRadius:8,padding:"10px 12px",fontSize:13,color:NV,textAlign:"center",fontWeight:700}}>{exportMsg}</div>}
          <div style={{background:"#fff",borderRadius:14,padding:16,marginTop:16,border:"2px dashed #d1d5db"}}>
            <p style={{margin:"0 0 6px",fontWeight:800,fontSize:13,color:"#333",letterSpacing:1}}>🧪 DEMO / SAMPLE DATA</p>
            <p style={{margin:"0 0 12px",fontSize:13,color:"#333",fontWeight:500}}>Load 20 sample work orders to preview reports. Clear when going live.</p>
            ...
            {workOrders.filter(w=>w.isSampleData).length>0&&<p style={{margin:"10px 0 0",fontSize:13,color:"#059669",textAlign:"center",fontWeight:700}}>✓ {workOrders.filter(w=>w.isSampleData).length} sample tickets loaded</p>}
          </div>
        </>}
      </div>
...
function WorkOrderForm({driver,quarries,companies,savedLocations=[],onSubmit,onCancel}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:"16px 20px 100px"}}>
        <Section title="WORK ORDER INFO" icon="📋">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="Date"><input type="date" value={form.date} onChange={e=>set("date",e.target.value)} style={S.inp}/></Field>
            <Field label="WO Number"><input value={form.workOrderNumber} onChange={e=>set("workOrderNumber",e.target.value)} style={S.inp}/></Field>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="Equipment Type"><select value={form.equipmentType} onChange={e=>set("equipmentType",e.target.value)} style={S.sel}>{EQUIP_TYPES.map(t=><option key={t}>{t}</option>)}</select></Field>
            <Field label="Truck Number" err={errors.truckNumber}><select value={form.truckNumber} onChange={e=>set("truckNumber",e.target.value)} style={{...S.sel,borderColor:errors.truckNumber?R:"#aaa"}}><option value="">-- Select --</option>{TRUCKS.map(t=><option key={t}>{t}</option>)}</select></Field>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="Job Name"><input value={form.jobName} onChange={e=>set("jobName",e.target.value)} style={S.inp} placeholder="Job name"/></Field>
            <Field label="Job Number"><input value={form.jobNumber} onChange={e=>set("jobNumber",e.target.value)} style={S.inp} placeholder="Job #"/></Field>
          </div>
        </Section>
        <Section title="CUSTOMER / COMPANY" icon="🏢">
          <Field label="Company Worked For" err={errors.companyWorkedFor}>
            {companies.length>0&&<select value={form.companyWorkedFor} onChange={e=>handleCompanyPick(e.target.value)} style={{...S.sel,marginBottom:8,borderColor:errors.companyWorkedFor?R:"#aaa"}}><option value="">-- Select saved company --</option>{companies.map(c=>{const n=getName(c);return <option key={n} value={n}>{n}</option>;})}</select>}
            <input value={form.companyWorkedFor} onChange={e=>set("companyWorkedFor",e.target.value)} style={{...S.inp,borderColor:errors.companyWorkedFor?R:"#aaa"}} placeholder={companies.length>0?"Or type new company name":"Company name"}/>
          </Field>
          {(form.companyContactName||form.companyBillingName)&&(<div style={{background:"#EEF2FF",borderRadius:8,padding:10,marginBottom:12,fontSize:13,color:"#1a1a1a",fontWeight:600}}>{form.companyContactName&&<div>👤 <strong>{form.companyContactName}</strong>{form.companyContactPhone?" — "+form.companyContactPhone:""}{form.companyContactEmail?" | "+form.companyContactEmail:""}</div>}{form.companyBillingName&&<div style={{marginTop:4}}>💳 Billing: <strong>{form.companyBillingName}</strong>{form.companyBillingPhone?" — "+form.companyBillingPhone:""}</div>}</div>)}
        ...
        <Section title="MATERIAL HAULED" icon="⛏️">
          <Field label="Material Category" err={errors.topMaterial}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:12}}>
              {TOP_MATERIALS.map(m=><button key={m} onClick={()=>setMany({topMaterial:m,materialHauled:"",quarry:"",quarryKnown:null,quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:""})} style={{padding:"10px 8px",border:`2px solid ${form.topMaterial===m?R:"#ccc"}`,borderRadius:10,background:form.topMaterial===m?R:"#fff",color:form.topMaterial===m?"#fff":"#1a1a1a",fontSize:13,fontWeight:700,cursor:"pointer"}}>{m}</button>)}
            </div>
          </Field>
          ...
          {needsQuarry&&form.topMaterial&&(
            <div style={{background:"#FFF8F0",border:"2px solid #FBBF24",borderRadius:10,padding:14,marginTop:4}}>
              <p style={{margin:"0 0 10px",fontWeight:800,color:"#92400E",fontSize:13}}>🪨 Do you know the quarry / source?</p>
              <div style={{display:"flex",gap:10,marginBottom:12}}>{[{label:"Yes – Select Quarry",val:true},{label:"Skip",val:false}].map(opt=><button key={String(opt.val)} onClick={()=>{set("quarryKnown",opt.val);if(!opt.val)setMany({quarry:"",quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:"",});}} style={{flex:1,padding:10,border:`2px solid ${form.quarryKnown===opt.val?R:"#ccc"}`,borderRadius:8,background:form.quarryKnown===opt.val?R:"#fff",color:form.quarryKnown===opt.val?"#fff":"#1a1a1a",fontWeight:700,cursor:"pointer",fontSize:13}}>{opt.label}</button>)}</div>
              {form.quarryKnown===true&&<>
                <Field label="Select Quarry">
                  <select value={form.quarry} onChange={e=>{if(e.target.value==="__add__")setShowNewQuarry(true);else setMany({quarry:e.target.value,quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:""});}} style={S.sel}>
                    <option value="">-- Select Quarry --</option>
                    {localQuarries.map(q=>{const n=getName(q);return <option key={n} value={n}>{n}</option>;})}
                    <option value="__add__">+ Add New Quarry...</option>
                  </select>
                </Field>
                {form.quarry&&(qObj||qDB)&&(<div style={{background:"#fff",borderRadius:8,padding:10,marginBottom:10,fontSize:13,color:"#222",border:"2px solid #e5e7eb",fontWeight:500}}><div style={{fontWeight:800,color:"#1a1a1a",fontSize:14}}>{form.quarry}</div>{(qObj?.address||qDB?.address)&&<div style={{color:"#333"}}>{qObj?.address||qDB?.address}</div>}<div style={{display:"flex",gap:12,marginTop:2,flexWrap:"wrap",color:"#333"}}>{(qObj?.phone||qDB?.phone)&&<span>📞 {qObj?.phone||qDB?.phone}</span>}{(qObj?.email||qDB?.email)&&<span>✉️ {qObj?.email||qDB?.email}</span>}{qObj?.text&&<span>💬 {qObj.text}</span>}</div>{qObj?.ap?.name&&<div style={{marginTop:4,color:"#166534",fontWeight:700}}>💳 AP: {qObj.ap.name}{qObj.ap.phone?" — "+qObj.ap.phone:""}</div>}{qObj?.dispatch?.name&&<div style={{color:NV,fontWeight:700}}>⚖️ {qObj.dispatch.name}{qObj.dispatch.phone?" — "+qObj.dispatch.phone:""}</div>}{(qObj?.notes||qDB?.note)&&<div style={{color:"#555",marginTop:2}}>{qObj?.notes||qDB?.note}</div>}</div>)}
                {form.quarry&&qDB&&<Field label="Stone / Material Type">
                  <select value={form.quarryMaterialName} onChange={e=>{const mat=qDB.materials.find(m=>m.name===e.target.value);if(mat)setMany({quarryMaterialName:mat.name,quarryMaterialCode:mat.code,quarryMaterialPrice:mat.price});else setMany({quarryMaterialName:"",quarryMaterialCode:"",quarryMaterialPrice:"",});}} style={S.sel}>
                    <option value="">-- Select stone type --</option>
                    {qDB.materials.map(m=><option key={m.code} value={m.name}>{m.name} (#{m.code}) — ${m.price.toFixed(2)}/ton</option>)}
                  </select>
                  {form.quarryMaterialName&&<div style={{display:"flex",gap:8,marginTop:8}}><div style={{flex:1,background:"#EEF2FF",borderRadius:8,padding:10,textAlign:"center"}}><div style={{fontSize:11,color:"#333",fontWeight:700}}>MATERIAL CODE</div><div style={{fontWeight:900,color:NV,fontSize:20}}>#{form.quarryMaterialCode}</div></div><div style={{flex:1,background:"#F0FDF4",borderRadius:8,padding:10,textAlign:"center"}}><div style={{fontSize:11,color:"#333",fontWeight:700}}>GATE RATE / TON</div><div style={{fontWeight:900,color:"#166534",fontSize:20}}>${form.quarryMaterialPrice}</div></div></div>}
                </Field>}
              </>}
            </div>
          )}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:12}}>
            <Field label="From Location"><LocationInput value={form.fromLocation} onChange={v=>set("fromLocation",v)} placeholder="Origin / quarry" savedLocations={savedLocations}/></Field>
            <Field label="To Location"><LocationInput value={form.toLocation} onChange={v=>set("toLocation",v)} placeholder="Job site" savedLocations={savedLocations}/></Field>
          </div>
        </Section>
        <Section title="TIME & HOURS" icon="⏱️">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="Time Arrived" err={errors.arrivedTime}><input type="time" value={form.arrivedTime} onChange={e=>set("arrivedTime",e.target.value)} style={{...S.inp,borderColor:errors.arrivedTime?R:"#aaa"}}/></Field>
            <Field label="Time Released" err={errors.releasedTime}><input type="time" value={form.releasedTime} onChange={e=>set("releasedTime",e.target.value)} style={{...S.inp,borderColor:errors.releasedTime?R:"#aaa"}}/></Field>
          </div>
          <Field label="Lunch Break?">
            <div style={{display:"flex",gap:10}}>{[{label:"✓ Worked Through",val:true},{label:"Took 30-min Break",val:false}].map(opt=><button key={String(opt.val)} onClick={()=>set("lunchWorked",opt.val)} style={{flex:1,padding:"10px 4px",border:`2px solid ${form.lunchWorked===opt.val?R:"#ccc"}`,borderRadius:8,background:form.lunchWorked===opt.val?R:"#fff",color:form.lunchWorked===opt.val?"#fff":"#1a1a1a",fontSize:13,fontWeight:700,cursor:"pointer"}}>{opt.label}</button>)}</div>
          </Field>
          <div style={{background:"#EEF2FF",borderRadius:10,padding:14,marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div><div style={{fontWeight:800,fontSize:13,color:NV,letterSpacing:0.5}}>STRAIGHT TIME HOURS</div><div style={{fontSize:13,color:"#333",fontWeight:600}}>Auto-calculated · edit if needed</div></div><div style={{fontSize:30,fontWeight:900,color:NV}}>{form.straightTime||"—"}</div></div>
            <input value={form.straightTime} onChange={e=>set("straightTime",e.target.value)} style={{...S.inp,textAlign:"center",fontWeight:700,fontSize:16}} placeholder="Enter hours manually"/>
          </div>
          <div style={{background:"#FFF7ED",borderRadius:10,padding:14,border:"2px solid #fed7aa"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontWeight:800,fontSize:13,color:"#92400E"}}>Overtime Pay?</div><div style={{fontSize:13,color:"#92400E",fontWeight:600}}>Enable only if this company pays OT</div></div>
              <div onClick={()=>set("overtimeEnabled",!form.overtimeEnabled)} style={{width:48,height:26,background:form.overtimeEnabled?R:"#d1d5db",borderRadius:13,cursor:"pointer",position:"relative",transition:"background 0.2s",flexShrink:0}}><div style={{position:"absolute",top:3,left:form.overtimeEnabled?25:3,width:20,height:20,background:"#fff",borderRadius:"50%",transition:"left 0.2s",boxShadow:"0 1px 3px rgba(0,0,0,0.3)"}}/></div>
            </div>
            {form.overtimeEnabled&&<div style={{marginTop:12}}><Field label="Overtime Hours"><input type="number" value={form.overtime} onChange={e=>set("overtime",e.target.value)} style={{...S.inp,textAlign:"center",fontWeight:700,fontSize:16}} placeholder="0.00" step="0.25" min="0"/></Field></div>}
          </div>
        </Section>
        <Section title="BILLING RATES" icon="💲">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="Rate Per Hour"><div style={{position:"relative"}}><span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#333",pointerEvents:"none",fontWeight:700}}>$</span><input type="number" value={form.ratePerHour} onChange={e=>set("ratePerHour",e.target.value)} style={{...S.inp,paddingLeft:26}} placeholder="0.00"/></div></Field>
            <Field label="Rate Per Load"><div style={{position:"relative"}}><span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#333",pointerEvents:"none",fontWeight:700}}>$</span><input type="number" value={form.ratePerLoad} onChange={e=>set("ratePerLoad",e.target.value)} style={{...S.inp,paddingLeft:26}} placeholder="0.00"/></div></Field>
          </div>
      ...
      {showNewQuarry&&<Modal title="Add New Quarry" onClose={()=>setShowNewQuarry(false)}>
        <input value={newQuarryName} onChange={e=>setNewQuarryName(e.target.value)} style={S.inp} placeholder="Quarry name & city"/>
        <button onClick={()=>{if(!newQuarryName.trim())return;const q={...mkQuarry(),name:newQuarryName.trim()};setLocalQuarries(p=>[...p,q]);setMany({quarry:q.name});setNewQuarryName("");setShowNewQuarry(false);}} style={{...S.btn,background:R,color:"#fff",width:"100%",marginTop:12}}>Save Quarry</button>
      </Modal>}
    </div>
  );
}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:20}}>
        <div style={{background:"#FFF8F0",border:"2px solid #FBBF24",borderRadius:12,padding:14,marginBottom:20}}><p style={{margin:0,fontSize:13,color:"#92400E",fontWeight:700}}>⚠️ Both Driver and Foreman must sign before submitting.</p></div>
        {[{key:"driver",label:"Driver Signature",sub:form.driver,sig:driverSig,setSig:setDriverSig},{key:"foreman",label:"Foreman Signature",sub:"Hand phone to foreman",sig:foremanSig,setSig:setForemanSig}].map(s=>(
          <div key={s.key} style={{background:"#fff",borderRadius:14,padding:20,marginBottom:16,boxShadow:"0 2px 12px rgba(0,0,0,0.08)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div><div style={{fontWeight:800,fontSize:15,color:"#1a1a1a"}}>{s.label}</div><div style={{color:"#333",fontSize:13,fontWeight:600}}>{s.sub}</div></div><span style={{background:s.sig?"#dcfce7":"#fee2e2",color:s.sig?"#166534":"#991b1b",fontSize:13,padding:"3px 10px",borderRadius:20,fontWeight:700}}>{s.sig?"✓ Signed":"Pending"}</span></div>
            {s.sig?<img src={s.sig} alt={s.label} style={{width:"100%",height:80,objectFit:"contain",border:"1px solid #eee",borderRadius:8}}/>:<button onClick={()=>setActiveCanvas(s.key)} style={{...S.btn,background:NV,color:"#fff",width:"100%",fontSize:16,padding:16}}>✍ Tap to Sign</button>}
            {s.sig&&<button onClick={()=>s.setSig(null)} style={{marginTop:6,background:"none",border:"none",color:R,fontSize:13,cursor:"pointer",fontWeight:700}}>Re-sign</button>}
          </div>
        ))}
...
function InvoicePreview({workOrder:wo,onClose,onNewOrder}){
  ...
  const InvRow=({label,value})=>value?<div style={{display:"flex",borderBottom:"1px solid #eee",padding:"8px 0",fontSize:14}}><div style={{width:130,color:"#333",flexShrink:0,fontStyle:"italic",fontWeight:700}}>{label}</div><div style={{fontWeight:700,color:"#111"}}>{value}</div></div>:null;
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:16}}>
        <div style={{background:"#fff",borderRadius:14,padding:24,boxShadow:"0 4px 24px rgba(0,0,0,0.12)",fontFamily:"Georgia,serif"}}>
          <div style={{textAlign:"center",borderBottom:"3px solid #1a1a1a",paddingBottom:12,marginBottom:14}}>
            ...
            <div style={{fontSize:14,color:"#222",fontWeight:600}}>25603 Bethel Lane • Dow, IL 62022</div>
            <div style={{display:"flex",justifyContent:"space-between",marginTop:10,fontSize:13}}><div><strong>Bob Sanders</strong><br/>618-818-8225</div><div style={{textAlign:"center",color:"#333",fontSize:13,lineHeight:1.7,fontWeight:600}}>ROCK • SAND • DIRT • ASPHALT<br/>GRAVEL • BUILDING MATERIALS<br/>STRUCTURAL DEMO</div><div style={{textAlign:"right"}}><strong>Glenn Sanders</strong><br/>618-779-6576</div></div>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:12,fontSize:13,fontWeight:600}}><span><strong>No.</strong> {wo.workOrderNumber}</span><span><strong>Date:</strong> {fmt(wo.date)}</span></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:20}}>
            {[{label:"Driver",sig:wo.driverSig,name:wo.driver},{label:"Foreman",sig:wo.foremanSig,name:"Foreman"}].map(s=><div key={s.label}><div style={{fontSize:13,color:"#222",marginBottom:4,fontWeight:800}}>{s.label} Signature</div><div style={{border:"1px solid #ccc",borderRadius:6,height:70,overflow:"hidden",background:"#fafafa"}}>{s.sig&&<img src={s.sig} alt={s.label} style={{width:"100%",height:"100%",objectFit:"contain"}}/>}</div><div style={{fontSize:13,color:"#222",borderTop:"1px solid #ccc",paddingTop:4,marginTop:4,fontWeight:700}}>{s.name}</div></div>)}
          </div>
          <div style={{marginTop:20,paddingTop:14,borderTop:"2px solid #1a1a1a",fontSize:10,color:"#333",textAlign:"center",lineHeight:1.6}}><strong>EXCLUSIVE USE OF VEHICLE ORDERED AND PAYMENT OF CHARGES GUARANTEED BY SHIPPER</strong><br/>TERMS: net 30 Days. Accounts over 30 days subject to a 2% service charge.</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:14}}>
          <button onClick={onClose} style={{...S.btn,background:"#fff",color:"#1a1a1a",border:"2px solid #ccc",fontWeight:700}}>View Queue</button>
          <button onClick={onNewOrder} style={{...S.btn,background:R,color:"#fff"}}>+ New Order</button>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:"16px 20px 80px"}}>
        <div style={{background:`linear-gradient(135deg,${DR},${R})`,borderRadius:16,padding:20,marginBottom:16,color:"#fff"}}>
          <div style={{fontSize:13,opacity:0.9,letterSpacing:1,marginBottom:12,fontWeight:700}}>TODAY'S HOURS SUMMARY</div>
          <div style={{display:"flex",gap:12}}>{[{val:totalST.toFixed(2),label:"Straight Hrs"},{val:totalOT.toFixed(2),label:"Overtime Hrs"},{val:fmtH(totalST+totalOT),label:"Total",h:true}].map(s=><div key={s.label} style={{flex:1,background:s.h?"rgba(255,255,255,0.28)":"rgba(255,255,255,0.18)",borderRadius:10,padding:14,textAlign:"center",border:s.h?"2px solid rgba(255,255,255,0.5)":"none"}}><div style={{fontSize:28,fontWeight:900}}>{s.val}</div><div style={{fontSize:13,opacity:0.9,marginTop:2,fontWeight:700}}>{s.label}</div></div>)}</div>
        </div>
        {todayWOs.length>0&&<div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:16,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}><div style={{fontSize:13,fontWeight:800,color:"#222",letterSpacing:1,marginBottom:10}}>TODAY'S TICKETS ({todayWOs.length})</div>{todayWOs.map((wo,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:i<todayWOs.length-1?"1px solid #f0f0f0":"none"}}><div><div style={{fontWeight:700,fontSize:13,color:"#1a1a1a"}}>{wo.workOrderNumber}</div><div style={{fontSize:13,color:"#333",fontWeight:600}}>{wo.companyWorkedFor} • {wo.truckNumber}</div></div><div style={{textAlign:"right",fontSize:13}}><div style={{fontWeight:700,color:NV}}>{wo.straightTime}h ST</div>{wo.overtimeEnabled&&wo.overtime&&<div style={{color:R,fontWeight:700}}>+{wo.overtime}h OT</div>}</div></div>)}</div>}
        <div style={{background:"#fff",borderRadius:14,padding:20,boxShadow:"0 2px 10px rgba(0,0,0,0.07)",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><span style={{fontSize:28}}>⛽</span><div><div style={{fontWeight:800,fontSize:16,color:"#1a1a1a"}}>Fuel Report</div><div style={{fontSize:13,color:"#333",fontWeight:600}}>Enter gallons pumped today</div></div>{saved&&<span style={{marginLeft:"auto",background:"#dcfce7",color:"#166534",fontSize:13,fontWeight:800,padding:"4px 10px",borderRadius:20}}>✓ Saved</span>}</div>
          ...
          <div style={{marginBottom:14}}>
            <label style={S.lbl}>Gallons of Fuel Added</label>
            <div style={{position:"relative"}}><input type="number" value={gallons} onChange={e=>{setGallons(e.target.value);setSaved(false);}} style={{...S.inp,fontSize:32,fontWeight:900,textAlign:"center",color:NV,padding:"16px 12px",height:80}} placeholder="0.0" step="0.1" min="0"/><span style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",fontSize:14,color:"#333",fontWeight:700,pointerEvents:"none"}}>gal</span></div>
            <div style={{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}>{["18.7","19.1","25.0","37.4","42.0","50.0"].map(v=><button key={v} onClick={()=>{setGallons(v);setSaved(false);}} style={{padding:"6px 12px",background:gallons===v?NV:"#e8e8e8",color:gallons===v?"#fff":"#1a1a1a",border:"none",borderRadius:8,fontSize:13,fontWeight:700,cursor:"pointer"}}>{v}</button>)}</div>
          </div>
        ...
        {showConfirm&&<div style={{background:"#dcfce7",border:"2px solid #86efac",borderRadius:12,padding:16,textAlign:"center"}}><div style={{fontSize:24,marginBottom:4}}>✅</div><div style={{fontWeight:800,color:"#166534",fontSize:15}}>End of Day Complete!</div><div style={{fontSize:13,color:"#15803d",marginTop:4,fontWeight:600}}>{fmtH(totalST+totalOT)} worked • {gallons} gal fuel logged</div></div>}
      </div>
    </div>
  );
}
...
function QueueScreen({workOrders,onBack,onView}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:20}}>
        <div style={{display:"flex",gap:8,marginBottom:16}}>{["all","complete","draft"].map(f=><button key={f} onClick={()=>setFilter(f)} style={{padding:"7px 18px",borderRadius:20,border:"none",background:filter===f?R:"#fff",color:filter===f?"#fff":"#1a1a1a",fontSize:13,fontWeight:700,cursor:"pointer",boxShadow:"0 1px 4px rgba(0,0,0,0.12)"}}>{f.charAt(0).toUpperCase()+f.slice(1)}</button>)}</div>
        {filtered.length===0&&<div style={{textAlign:"center",padding:40,color:"#333",fontWeight:700,fontSize:15}}>No work orders found</div>}
        {filtered.map((wo,i)=><div key={i} onClick={()=>onView(wo)} style={{background:"#fff",borderRadius:14,padding:16,marginBottom:12,boxShadow:"0 2px 10px rgba(0,0,0,0.08)",cursor:"pointer"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div><div style={{fontWeight:800,fontSize:15,fontFamily:"Georgia,serif",color:DR}}>{wo.workOrderNumber}</div><div style={{color:"#333",fontSize:13,marginTop:2,fontWeight:600}}>{fmt(wo.date)} • {wo.driver}</div><div style={{fontSize:14,marginTop:4,fontWeight:800,color:"#1a1a1a"}}>{wo.companyWorkedFor||"—"}</div><div style={{color:"#333",fontSize:13,fontWeight:600}}>{wo.truckNumber} • {wo.topMaterial}</div></div><div style={{textAlign:"right"}}><span style={{background:wo.status==="complete"?"#dcfce7":"#fef9c3",color:wo.status==="complete"?"#166534":"#854d0e",fontSize:13,padding:"3px 10px",borderRadius:20,fontWeight:800}}>{wo.status==="complete"?"✓ Signed":"Draft"}</span>{wo.straightTime&&<div style={{fontSize:13,color:"#222",marginTop:8,fontWeight:700}}>{wo.straightTime}h{wo.overtimeEnabled&&wo.overtime?" + "+wo.overtime+"h OT":""}</div>}</div></div></div>)}
      </div>
    </div>
  );
}
...
function ReportsScreen({workOrders,fuelLogs=[],drivers=[],onBack}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f4f4"}}>
      ...
      <div style={{padding:"16px 20px 80px"}}>
        <div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}>
          <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>{[["daily","Day"],["weekly","Week"],["monthly","Month"],["yearly","Year"]].map(([v,l])=><button key={v} onClick={()=>setView(v)} style={{padding:"7px 18px",borderRadius:20,border:"none",background:view===v?R:"#e8e8e8",color:view===v?"#fff":"#1a1a1a",fontWeight:700,fontSize:13,cursor:"pointer"}}>{l}</button>)}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:14}}>{[{label:"Work Orders",val:rWOs.length,color:DR,icon:"📋"},{label:"Straight Hrs",val:totalST.toFixed(2),color:NV,icon:"⏱"},{label:"Overtime Hrs",val:totalOT.toFixed(2),color:R,icon:"⚡"},{label:"Total Gallons",val:totalGal.toFixed(1),color:"#92400E",icon:"⛽"}].map(s=><div key={s.label} style={{background:"#fff",borderRadius:12,padding:14,textAlign:"center",boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}><div style={{fontSize:18,marginBottom:4}}>{s.icon}</div><div style={{fontSize:24,fontWeight:900,color:s.color}}>{s.val}</div><div style={{fontSize:13,color:"#333",marginTop:2,fontWeight:700}}>{s.label}</div></div>)}</div>
        {Object.keys(byDriver).length>0&&<div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}><div style={{fontSize:13,fontWeight:800,color:"#222",letterSpacing:1,marginBottom:12}}>DRIVER TOTALS — {pl.toUpperCase()}</div>{Object.entries(byDriver).sort((a,b)=>b[1].st-a[1].st).map(([n,d])=><div key={n} style={{padding:"14px 0",borderBottom:"1px solid #f0f0f0"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div style={{fontWeight:800,fontSize:15,color:"#1a1a1a"}}>{n}</div><div style={{textAlign:"right"}}><div style={{fontWeight:900,color:NV,fontSize:16}}>{(d.st+d.ot).toFixed(2)}h total</div><div style={{fontSize:13,color:"#333",fontWeight:700}}>{d.st.toFixed(2)} ST{d.ot>0?" • "+d.ot.toFixed(2)+" OT":""}</div></div></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><span style={{background:"#EEF2FF",color:NV,fontSize:13,fontWeight:700,padding:"3px 8px",borderRadius:6}}>📋 {d.count} tickets</span>{d.gallons>0&&<span style={{background:"#FFF7ED",color:"#92400E",fontSize:13,fontWeight:700,padding:"3px 8px",borderRadius:6}}>⛽ {d.gallons.toFixed(1)} gal</span>}{d.earnings>0&&<span style={{background:"#F0FDF4",color:"#166534",fontSize:13,fontWeight:700,padding:"3px 8px",borderRadius:6}}>💵 ${d.earnings.toFixed(0)}</span>}</div></div>)}</div>}
        {rFuel.length>0&&<div style={{background:"#fff",borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}><div style={{fontSize:13,fontWeight:800,color:"#222",letterSpacing:1,marginBottom:10}}>⛽ FUEL LOGS</div>{rFuel.sort((a,b)=>new Date(b.date)-new Date(a.date)).map((l,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"9px 0",borderBottom:i<rFuel.length-1?"1px solid #f0f0f0":"none"}}><div><div style={{fontWeight:700,color:"#1a1a1a"}}>{l.driver}</div><div style={{fontSize:13,color:"#333",fontWeight:600}}>{fmt(l.date)} • {l.truck}{l.notes?" • "+l.notes:""}</div></div><div style={{fontWeight:900,color:"#92400E",fontSize:16}}>{parseFloat(l.gallons).toFixed(1)} gal</div></div>)}</div>}
        {rWOs.length===0&&rFuel.length===0&&<div style={{textAlign:"center",padding:40,color:"#333",background:"#fff",borderRadius:14,fontWeight:600,fontSize:15}}>No data for this period</div>}
        <div style={{background:"#fff",borderRadius:14,padding:16,boxShadow:"0 2px 10px rgba(0,0,0,0.07)"}}>
          <div style={{fontSize:13,fontWeight:800,color:"#222",letterSpacing:1,marginBottom:12}}>EXPORT OPTIONS</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          {exportMsg&&<div style={{marginTop:12,background:exportMsg.startsWith("✅")||exportMsg.startsWith("📋")?"#F0FDF4":"#FEF2F2",border:`1px solid ${exportMsg.startsWith("✅")||exportMsg.startsWith("📋")?"#86efac":"#fecaca"}`,borderRadius:8,padding:"12px 14px",fontSize:13,fontWeight:700,color:exportMsg.startsWith("✅")||exportMsg.startsWith("📋")?"#166534":"#991b1b",textAlign:"center"}}>{exportMsg}</div>}
        </div>
      </div>
      <Toast msg={exportMsg}/>
    </div>
...
export default function App(){
  ...
  useEffect(()=>{(async()=>{try{const r1=await window.storage.get("rjs-work-orders");if(r1){const wos=JSON.parse(r1.value);setWorkOrders(wos);initWoNum(wos);}const r2=await window.storage.get("rjs-drivers");if(r2)setDrivers(JSON.parse(r2.value).map(normDriver));const r3=await window.storage.get("rjs-quarries");if(r3)setQuarries(JSON.parse(r3.value).map(normQuarry));const r4=await window.storage.get("rjs-companies");if(r4)setCompanies(JSON.parse(r4.value).map(normCompany));const r5=await window.storage.get("rjs-locations");if(r5)setSavedLocations(JSON.parse(r5.value));const r6=await window.storage.get("rjs-fuel-logs");if(r6)setFuelLogs(JSON.parse(r6.value));}catch(e){}})();},[]);
